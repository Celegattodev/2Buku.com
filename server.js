...
app.get('/catalog-data', (req, res) => {
  ...
  const popularBooksSql = `
    SELECT l.google_books_id, l.user_id, COUNT(*) as count
    FROM livros l
    JOIN users u ON l.user_id = u.id
    WHERE l.status = 'Disponível' AND u.status = 'ativo'
    GROUP BY l.google_books_id, l.user_id
    ORDER BY count DESC
    LIMIT 10
  `;
  ...
  const latestBooksSql = `
    SELECT l.google_books_id, l.user_id
    FROM livros l
    JOIN users u ON l.user_id = u.id
    WHERE l.status = 'Disponível' AND u.status = 'ativo'
...
app.get('/search-books', async (req, res) => {
  ...
  try {
    const searchSql = `
      SELECT l.id, l.titulo, l.autor, l.imagem AS imageUrl, l.google_books_id AS googleBooksId, l.user_id AS userId
      FROM livros l
      JOIN users u ON l.user_id = u.id
      WHERE (l.titulo LIKE ? OR l.autor LIKE ?) AND l.status = "Disponível" AND u.status = "ativo"
    `;
